...
class RoleController extends AdminController
{
    ...
    public function userList($page = 1, $r = 20)
    {
        ...
        if($aSingleRole){//单身份筛选
            $uids=$this->D('MyRoles')->group('uid')->field('uid')->having('count(uid)=1')->select();
        ...
        if (!is_administrator()) $map_user_list['aid'] = array('eq',session('user_auth.aid')); 
        $user_list = D('MyRoles')->where($map_user_list)->page($page, $r)->order('uid desc')->select();
        $totalCount = D('MyRoles')->where($map_user_list)->count();
