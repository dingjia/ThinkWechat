    ...
    public function upload($files, $setting, $driver = 'local', $config = null){
        ...
        if($info){ //文件上传成功，记录文件信息
            foreach ($info as $key => &$value) {
               ...
               $pic_path="http://".$_SERVER[HTTP_HOST].__ROOT__.$value['path'];
            
             
               $my_info = getimagesize($pic_path);
               $exif  =  GetImageInfo($pic_path);
               $value['width'] = $my_info[0];
               $value['height'] = $my_info[1];
               $value['channels'] = $my_info['channels'];

              
               if (count($exif)>2)$value['exif'] = json_encode($exif);
