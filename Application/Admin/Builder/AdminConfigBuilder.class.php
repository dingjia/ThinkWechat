...
class AdminConfigBuilder extends AdminBuilder
{
     ...
     * @auth 陈一枭
     */
    public function key($name, $title, $subtitle = null, $type, $opt = null,$inline=10)
    {
        $key = array('name' => $name, 'title' => $title, 'subtitle' => $subtitle, 'type' => $type, 'opt' => $opt,'inline' => $inline);
        $this->_keyList[] = $key;
        return $this;
    }
     ...
     * @auth 陈一枭
     */
    public function keyHidden($name, $title, $subtitle = null,$inline=null)
    {
     ...
     * @auth 陈一枭
     */
    public function keyReadOnly($name, $title, $subtitle = null,$inline=null)
    {
        return $this->key($name, $title, $subtitle, 'readonly',$opt = null,$inline);
    }
     ...
     * @auth 陈一枭
     */
    public function keyText($name, $title, $subtitle = null, $opt = null,$inline=null)
    {
        return $this->key($name, $title, $subtitle, 'text',$opt = null,$inline);
    }
    ...
    public function keySelect($name, $title, $subtitle = null, $options,$inline=10)
    {
        return $this->key($name, $title, $subtitle, 'select', $options,$inline);
    }

    public function keyRadio($name, $title, $subtitle = null, $options,$inline = null)
    {
        return $this->key($name, $title, $subtitle, 'radio', $options,$inline );
    }
    ...
    public function keyProgressbar($name, $title, $subtitle = null, $options)
    {
        return $this->key($name, $title, $subtitle, 'progressbar', $options);
    }

   public function keyTree($name, $title, $subtitle = null, $options)
    {
        return $this->key($name, $title, $subtitle, 'tree', $options);
    }

  
    public function keyEditor($name, $title, $subtitle = null, $config = '', $style = array('width' => '100%', 'height' => '200px'))
    {
     ...
     * @author 郑钟良<zzl@ourstu.com>
     */
    public function keyTime($name, $title, $subtitle = null,$type='datetime',$inline=null)
    {
        return $this->key($name, $title, $subtitle, $type,'',$inline);
    }

    public function keyCreateTime($name = 'create_time', $title = '创建时间', $subtitle = null,$inline=null)
    {
        return $this->keyTime($name, $title, $subtitle,'datetime',$inline);
    }

    public function keyBool($name, $title, $subtitle = null,$inline=null)
    {
        $map = array(1 => L('_YES_'), 0 => L('_NO_'));
        return $this->keyRadio($name, $title, $subtitle, $map,$inline);
    }
    ...
    public function keyId($name = 'id', $title = '编号', $subtitle = null,$inline=null)
    {
     ...
     * @param null $subtitle
     */
    public function keySingleFile($name, $title, $subtitle = null,$inline=null){
        return  $this->key($name,$title,$subtitle,'singleFile','',$inline);
    }
    ...
    public function keySingleImage($name, $title, $subtitle = null,$inline=null)
    {
        return $this->key($name, $title, $subtitle, 'singleImage','',$inline);
    }
    ...
    public function keySingleUserGroup($name, $title, $subtitle = null)
    {
     ...
     public function keySingleUserRoles($name, $title, $subtitle = null)
    {
        $options = $this->readUserRoles();
        return $this->keySelect($name, $title, $subtitle, $options);
    }
    ...
    // }
      public function keyAddress($name, $title, $subtitle = null, $options)
    {
        return $this->key($name, $title, $subtitle, 'address', $options);
    }
     ...
     * @author @MingYangliu <xint5288@126.com>
     */
    public function keyLoctionMap($name, $title, $subtitle = null, $url)
    {
       
        return $this->key($name, $title, $subtitle, 'loctionMap', $urls);
    }
     ...
     * @author:xjw129xjt(肖骏涛) xjt@ourstu.com
     */
    public function keyChosenOne($name, $title, $subtitle = null, $options)
    {
       
       
        return $this->key($name, $title, $subtitle, 'chosenone', $options);
    }
    ...
    public function handleConfig()
    {
        if (IS_POST) {
            ...
            foreach (I('') as $k => $v) {
                ...
                $config['sort'] = 0;
                $config['aid'] = session('user_auth.aid');

                $map['aid']=session('user_auth.aid');
                $map['name']=$config['name'];
                $have=$configModel->where($map)->getfield('id');
                if ($have){
                    $res=$configModel->where(array('id'=>$have))->save($config);
                }else{
                    $res=$configModel->add($config);
                }
              
                $tag = 'conf_' . strtoupper(CONTROLLER_NAME) . '_' . strtoupper($k).'_'.session('user_auth.aid');
                S($tag, null);
            }
            if ($res>0) {
                if ($this->_callback) {
            ...
            $map['name'] =array('like', '_' . strtoupper(CONTROLLER_NAME) . '_' . '%');
            $configs = D('Config')->where($map)->limit(999)->select();
            $data = array();
    ...
    private function readUserRoles()
    {
        $list = M('Role')->where(array('status' => 1))->order('id asc')->select();
        $result = array();
