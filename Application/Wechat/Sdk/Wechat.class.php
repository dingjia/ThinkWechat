...
class Wechat
{
	...
	const BATCH_USER_INFO_URL='/user/info/batchget?';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-1800 : 3600;  //提前半个小时
	...
	public function batchgetUserInfo($data){
		...
		foreach ($data as $key => $value) {
			$newdata[]['openid']=$value;
		}
		$json['user_list']=$newdata;
		
		$result = $this->http_post(self::API_URL_PREFIX.self::BATCH_USER_INFO_URL.'access_token='.$this->access_token,self::json_encode($json));
		if ($result)
		{
			$json = json_decode($result,true);
			if (isset($json['errcode'])) {
     ...
     * }
     */
    public function userCardlist($openid,$card_id='') {
