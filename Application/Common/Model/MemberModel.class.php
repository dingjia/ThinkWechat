...
class MemberModel extends Model
{
    ...
    public function registerMember($nickname = '')
    {
        ...
        $user_arr=array( 'nickname' => $nickname,  'status' => 1);
        if (!is_administrator()) $user_arr['aid'] = session('user_auth.aid');
        if ($user = $this->create($user_arr)) {
            $uid = $this->add($user);
            if (!$uid) {
                $this->error = L('_THE_FOREGROUND_USER_REGISTRATION_FAILED_PLEASE_TRY_AGAIN_WITH_EXCLAMATION_');
                return false;
            }
            //如果是超级管理添加的集团号，则AID就是起id
            if (is_administrator()) $this->where(array('uid'=>$uid))->save(array('aid'=>$uid));
            $this->initFollow($uid);
            return $uid;
        } else {
    ...
    private function autoLogin($user, $remember = false, $role_id = 0)
    {
        ...
        $auth = array(
            ...
            'mobile'   =>query_user('mobile',$user['uid']) ,
