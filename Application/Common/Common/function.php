...
use Wechat\Model\WechatSdkModel;
...
function is_login()
{
    ...
    if (empty($user)) { 
        //如果是在微信，并且没有登录，还带有一个openid,则登录检查是不是自己，如果不是则是这个人分享，如果是就是自己。
        if (is_weixin() and $_SESSION['wxuser']['nickname'])  return $_SESSION['wxuser']['openid'];
        return false;
    } else {
