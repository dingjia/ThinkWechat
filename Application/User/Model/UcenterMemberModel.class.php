...
class UcenterMemberModel extends Model
{
     ...
     * @return array                用户信息
     */
    public function info($uid, $is_username = false,$is_openid = false)
    {
        $map = array();
        ...
        $user = $this->where($map)->field('id,username,email,mobile,status,openid')->find();
        if (is_array($user) && $user['status'] = 1) {
            return array($user['id'], $user['username'], $user['email'], $user['mobile'],$user['openid']);
        } else {
            return -1; //用户不存在或被禁用
        }
    }
